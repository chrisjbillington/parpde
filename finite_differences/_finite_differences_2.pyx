...
@cython.nonecheck(False)
@cython.cdivision(True)
cdef inline psi_t operators_single_point_interior(
    ...
    psi_t [:, :] left_buffer, psi_t [:, :] right_buffer, psi_t [:, :] bottom_buffer, psi_t [:, :] top_buffer):
...
def SOR_step(
    ...
    gradx_coeff_t [:, :] gradx_coeff, grady_coeff_t [:, :] grady_coeff,
    grad2x_coeff_t [:, :] grad2x_coeff, grad2y_coeff_t [:, :] grad2y_coeff,
    b_t [:, :] b, double dx, double dy, double relaxation_parameter,
    int operator_order, double squared_error=0, int interior=1, int edges=1,
    ...
    while True:
        ...
        A_hollow_psi = operators_single_point_interior(
